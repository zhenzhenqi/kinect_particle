...
class PolygonBlob extends Polygon {
  ...
  void createPolygon() {
    ...
    for (int n=0; n<theBlobDetection.getBlobNb(); n++) { 
      Blob b = theBlobDetection.getBlob(n); // for each substantial blob... if (b != null && b.getEdgeNb() > 100) {
    ...
    while (contours.size() > 0) {
      ...
      for (Object p : contour) {
        PVector pv = (PVector)p;
        addPoint(int(pv.x), int(pv.y));
      }
